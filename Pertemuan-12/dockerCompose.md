...
9. Lakukan Testing di Local Project Next.JS - Install Dependencies: `npm install` - Create user di DBMS : `sudo mysql -u root -p` - `CREATE USER 'userwebdinamis_nim'@'localhost' IDENTIFIED BY 'O)xz6GWEwDOx1Ea9';` - `GRANT ALL PRIVILEGES ON *.* TO 'userwebdinamis_nim'@'localhost';` - `FLUSH PRIVILEGES;` - `exit;`
   ![alt text](image-1.png) - Edit File .env di folder web-dinamis - `npm run build` - `npm start` - Pastikan web dapat diakses di http://localhost:3000 admin tanpa error
